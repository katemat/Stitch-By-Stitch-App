def create_image_array_by_project_id(id)
    run_sql("INSERT INTO images(image_arr, project_id) VALUES ($1, $2)", [image_arr, project_id])
end
...
def all_images_by_project_id(id)
    sql = "SELECT * FROM images WHERE EXISTS (SELECT * from images WHERE project_id = $1)";
    run_sql(sql, [id])[0]
  end